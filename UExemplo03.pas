...
type
  Tfrm_exemplo03 = class(TForm)
    ...
    Button4: TButton;
    Button5: TButton;
    BtnMutiplicacao: TButton;// (X)
    BtnDivisao: TButton; // (/)
    BtnAdicao: TButton; // (+)
    BtnSubtracao: TButton; // (-)
    ButtonVg: TButton; // (,)
    BtnResultado: TButton; // (=)
    BtnLimpar: TButton; //limpar visor
    Visor: TEdit;  // entrada/saída (visor)
    memHistorico: TMemo;  //Hitóico das operações
  ...
  private
    valoratual: double;  //guarda o primeiro valor digitado
    operacao: integer; //código da operãção escolhida (1=+, 2=-, 3=*, 4=/)
    limpar: boolean;   //flag apagar visor
...
{$R *.dfm}
    //adiciona uma linha ao histórico com a oreração realizada
...
begin
  ...
  case operacao of
    1: resultado := valoratual + valor2;   //adição
    2: if valor2 <> 0 then    //divisão com verificação
         resultado := valoratual / valor2
       else begin
         ShowMessage('Erro: divisão por zero!');
         Exit;
       end;
    3: resultado := valoratual * valor2;  //Multiplicação
    4: resultado := valoratual - valor2;  // Subtração
  end;
   // Atualiza histórico e visor
  ...
  // Limpa visor, histórico e variáveis
...
begin
   ...
   memHistorico.Clear;
               ...
               {Evento de Click }
    //adiciona o dígito correspondente ao visor
   //o visor é apagado se a flag "limpa" estiver ativa
